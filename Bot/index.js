...
//Gas Limit  BigNumber { value: "281752" }
// gas limit 236630
...
const runBot = async () => {
  ...
  provider.on('block', async (blockNumber) => {
    try {
      ...
      /*
      const spread = Math.abs(priceJoe - pricePangolin)

      let priceOut = (shouldStartAvax ? 
        (AVAX_TRADE_FEE * reserve1Pan) / (AVAX_TRADE_FEE + reserve0Pan) / AVAX_TRADE
        : PAIR_TRADE / ((PAIR_TRADE_FEE * reserve0Pan) / (PAIR_TRADE_FEE + reserve1Pan)) )
      console.log("Price Out ", priceOut)
      
      // Calculate how much you will get out after fees and slippage
      let amountOut = (shouldStartAvax ? 
        (AVAX_TRADE_FEE * reserve1Pan) / (AVAX_TRADE_FEE + reserve0Pan) 
        : ((PAIR_TRADE_FEE * reserve0Pan) / (PAIR_TRADE_FEE + reserve1Pan)) )
      ...
      */
      ...
      //the difference between what is pulled out of joe and what is need to put into Pan to get what is needed to go to back to Joe
      ...
      console.log("Gas Limit ", Number(gasLimit))
      ...
      console.log("compared to AVAX Profit of", (shouldStartAvax ?  profit : profit / priceJoe ))
